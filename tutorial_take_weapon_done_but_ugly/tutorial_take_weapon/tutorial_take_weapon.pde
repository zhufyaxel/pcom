...
Tutorial_take_weapon tutorial;


void setup() {
  size(1024, 768, P2D);
  tutorial = new Tutorial_take_weapon();
}
...
class Tutorial_take_weapon {
  ...
  // sound
  Minim minim;
  AudioPlayer magic, swipe, defend;
  Note dol, mi, sol;
  ...
  Tutorial_take_weapon() {
    ...
    // sound (Minim!!)
    minim = new Minim(tutorial_take_weapon.this);
    magic = minim.loadFile("music/magic.mp3", 512);
    swipe = minim.loadFile("music/swipe.mp3", 512);
    defend = minim.loadFile("music/shield.mp3", 512);
